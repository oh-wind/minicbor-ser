...
impl <'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    where
        ...
        match self.decoder.datatype()? {
            ...
            Type::Simple => Err(type_mismatch(Type::Simple, "rust doesn't support this type")),
            ...
            Type::Break => Err(type_mismatch(Type::Break, "break stop code outside indefinite length item")),
            Type::Unknown(u) => Err(type_mismatch(Type::Unknown(u), "rust doesn't support this type")),
        }
    }
    ...
    where
        ...
        match self.decoder.datatype()? {
            ...
            _ => {
                Err(type_mismatch(Type::Null, "expected unit(null)"))
            }
        }
    }
    ...
    where
        ...
        match self.decoder.datatype()? {
            ...
            _ => {
                Err(type_mismatch(Type::Map, "expected map"))
            }
        }
    }
    ...
    where
        ...
        match self.decoder.datatype()? {
            Type::String | Type::StringIndef => {
                visitor.visit_enum(EnumUnitAccess::new(self))
            }
            Type::Map | Type::MapIndef => {
            ...
            t@_ => {
                Err(type_mismatch(t, "expected map or string"))
            }
        }
    }
...
impl <'a, 'de> de::MapAccess<'de> for MapAccess<'a, 'de> {
    ...
    where
        ...
        match decoder.datatype()? {
            Type::Break => {
                Err(type_mismatch(Type::Break, "expect value, but found break stop code(0xFF)."))
            }
            _ => {
